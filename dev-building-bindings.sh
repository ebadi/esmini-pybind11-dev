...
do
	...
	~/binder/bin/binder --annotate-includes --trace --root-module py$MYMODULE --prefix $PWD/esmini/src/$MYMODULE --bind $MYMODULE  --single-file  --config $PWD/esmini/src/py$MYMODULE.cfg  $PWD/esmini/src/py$MYMODULE.hpp  -- -std=c++14 -DNDEBUG $IPATHS
done

MYMODULE="viewerbase"
rm -rf $PWD/esmini/src/$MYMODULE ;  mkdir  $PWD/esmini/src/$MYMODULE
~/binder/bin/binder --annotate-includes --trace --root-module py$MYMODULE --prefix $PWD/esmini/src/$MYMODULE --bind viewer --single-file  --config $PWD/esmini/src/py$MYMODULE.cfg  $PWD/esmini/src/py$MYMODULE.hpp  -- -std=c++14 -DNDEBUG $IPATHS
...
do
	echo $MYMODULE
	rm -rf $PWD/esmini/src/$MYMODULE ;  mkdir  $PWD/esmini/src/$MYMODULE
	~/binder/bin/binder --annotate-includes --trace --root-module py$MYMODULE --prefix $PWD/esmini/src/$MYMODULE --bind ""  --single-file  --config $PWD/esmini/src/py$MYMODULE.cfg  $PWD/esmini/src/py$MYMODULE.hpp  -- -std=c++14 -DNDEBUG $IPATHS
done
